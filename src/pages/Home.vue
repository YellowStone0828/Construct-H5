  ...
  <a-layout class="pageAll">
    <a-layout-sider
      ...
      <a-menu theme="dark" mode="inline" :open-keys="openNavList" @openChange="onOpenNav">
        <template v-for="(nav,nInd) in NavBarData">
        <a-sub-menu v-if="nav.Children != null && nav.Children.length > 0" :key="nav.MenuID" >
          ...
          <a-menu-item v-else :key="nav.MenuID">
            <router-link :to="nav.MenuPath"><a-icon :type="nav.Icon"/>{{ nav.MenuTitle }}</router-link>
          </a-menu-item>
        </template>
      </a-menu>
    </a-layout-sider>
    <a-layout>
      <a-layout-header class="pageHead">
        ...
        <a-switch class="openMethod headTool" checked-children="Tab" un-checked-children="Cover"
                  :checked="isOpenRemember" @change="changeOpenType"/>
